...
def print_schedule(data, first_path_sol, second_path_sol, ind):
    ...
    f=open(f"reb_schedule_{ind}.txt","w")
    ...
    f=open(f"reb_schedule_of_teachers_{ind}.txt","w")
...
def get_objVal(groups):



    course_gr = [[] for __ in range(13)]
    ...
    penalty = sum([F[gr[2],gr[1] - 1] for gr in groups])
...
def get_up_bound(data):

    B = get_list_of__student(data)
    gr_num = np.zeros((L),dtype= np.int8)
    num_st = J
    for course in range(L):
        gr_num[course] = B[course] // 8
        if B[course] % 8 >= 3:
            gr_num[course]+=1
        else:
            num_st-= B[course] % 8
    ...
    # sum_timeslot_up_bound = 0
    # for course in range(L):
    #     sum_timeslot_up_bound+=gr_num[course]*timeL[course]  
