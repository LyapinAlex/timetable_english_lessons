...
def check_rooms(group, t_1, t_2, data, schedule):
    ...
    for tReal in range(timeLessons[l]):
        ...
        if np.sum([ schedule_of_teachers[i, work_time[0][0], t_1 + tReal] for i in range(I) ]) >= r:
            return False

        if np.sum([ schedule_of_teachers[i, work_time[1][0], t_2 + tReal] for i in range(I) ]) >= r:
            return False
...
#     real_time = schedule['real_time']
#     schedule_of_teachers =  schedule['schedule_of_teachers']
#     # schedule_of_teachers[i, work_time[0][0], t_1 + tReal]
#     l = group[2]
 
#     work_time = group[3], group[4]
    
#     for tReal in range(timeLessons[l] + 1):
#         if (t_1 + tReal == real_time) or ( t_2  + tReal == real_time):
#             continue


#         if  t_1 + tReal == 0:
#             if  schedule_of_teachers[i, work_time[0][0], 1] == 1:
#                 return False
#         elif  t_1 + tReal == real_time - 1:
#             if  schedule_of_teachers[i, work_time[0][0],  real_time - 2] == 1:
#                 return False
#         else:
#             if ( schedule_of_teachers[i, work_time[0][0], t_1 + tReal - 1] == 1) or (schedule_of_teachers[i, work_time[0][0], t_1 + tReal + 1] == 1):
#                 return False


#         if  t_2 + tReal == 0:
#             if  schedule_of_teachers[i, work_time[1][0], 1] == 1:
#                 return False
#         elif  t_2 + tReal == real_time - 1:
#             if  schedule_of_teachers[i, work_time[1][0],  real_time - 2] == 1:
#                 return False
#         else:
#             if ( schedule_of_teachers[i, work_time[1][0], t_2 + tReal - 1] == 1) or (schedule_of_teachers[i, work_time[1][0], t_2 + tReal + 1] == 1):
#                 return False
